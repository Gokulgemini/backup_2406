...
inputs = {
  function_name = "${local.env}-${local.app_env}-dpxn-check-order"
  bucket_name = dependency.s3-dpxn-check-order.outputs.s3_bucket_id
  queue_url = dependency.sqs-dpxn-check-order.outputs.queue_url
  ...
  "Statement": [
      {
        ...
        "Resource": "${dependency.sqs-dpxn-check-order.outputs.queue_arn}"
      },{
      "Action": [
        "kms:Encrypt",
        "kms:Decrypt",
      ...
      "Effect": "Allow",
      "Resource": "${dependency.kms-dpxn-check-order.outputs.kms_arn}"
    }
  ]
}
EOF
