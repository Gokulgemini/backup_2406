...
dependency "logstream-dpxn-aim-api-cloudwatch-log-group" {
...
inputs = {
  ...
  create_role = true
  create_policy = true
  role_name = "${local.infra_environment}-${local.app_environment}-aim-api-role"
  role_description = "Role used By DLX AIM.API Service (${local.app_environment})"
  policy_name = "${local.infra_environment}-${local.app_environment}-aim-api-policy"
  policy_description = "Policy used By DLX AIM.API Service IAM Role (${local.app_environment})"
  role_path = "/"
  policy_statement = templatefile("policy/statement.tftpl", {
                      logstream-dpxn-aim-api-cloudwatch-log-group-arn = dependency.logstream-dpxn-aim-api-cloudwatch-log-group.outputs.cloudwatch_log_group_arn
  })
  oidc_fully_qualified_subjects = [
    "system:serviceaccount:external:aim-api"
  ]
