...
dependency "sqs-dpxn-print-scan-notifications" {
  config_path = "${get_terragrunt_dir()}/../sqs-dpxn-print-scan-notifications"
...
inputs = {
  ...
  create_role             = true
  create_policy           = true
  role_name               = "${local.chef_environment}-${local.app_environment}-print-scan-notifications-role"
  role_description        = "Role used by DPXN Print Scan Notifications Service (${local.app_environment})"
  policy_name             = "${local.chef_environment}-${local.app_environment}-print-scan-notifications-policy"
  policy_description      = "Policy used by DPXN Print Scan Notifications Service Role (${local.app_environment})"
  policy_statement        = templatefile("policy/statements.tftpl", {
    sqs-dpxn-print-scan-notifications           = dependency.sqs-dpxn-print-scan-notifications.outputs.queue_arn,
    s3-dpxn-print-scan-notifications            = dependency.s3-dpxn-print-scan-notifications.outputs.s3_bucket_arn,
    s3-dpxn-check-order                         = dependency.s3-dpxn-check-order.outputs.s3_bucket_arn,
    kms-dpxn-print-scan-notifications           = dependency.kms-dpxn-print-scan-notifications.outputs.kms_arn,
    kms-dpxn-check-order                        = dependency.kms-dpxn-check-order.outputs.kms_arn
  })
  oidc_fully_qualified_subjects   = [
    "system:serviceaccount:print-scan-service-layer:dlx-print-scan-notifications"
  ]
