...
inputs = {
  function_name          = "${local.chef_environment}-${local.app_environment}-print-scan-notifications"
  bucket_name            = dependency.s3-dpxn-print-scan-notifications.outputs.s3_bucket_id
  queue_url              = dependency.sqs-dpxn-print-scan-notifications.outputs.queue_url
  ...
  "Statement": [
      {
        ...
        "Resource": "${dependency.sqs-dpxn-print-scan-notifications.outputs.queue_arn}"
      },{
      "Action": [
        "kms:Encrypt",
        "kms:Decrypt",
      ...
      "Effect": "Allow",
      "Resource": "${dependency.kms-dpxn-print-scan-notifications.outputs.kms_arn}"
    }
  ]
}
EOF
