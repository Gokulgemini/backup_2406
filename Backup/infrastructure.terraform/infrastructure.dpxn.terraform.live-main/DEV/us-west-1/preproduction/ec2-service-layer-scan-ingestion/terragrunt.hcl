...
dependency "iam-role-ec2-dpxn-service-layer-scan-ingestion" {
  config_path = "${get_terragrunt_dir()}/../iam-role-ec2-dpxn-service-layer-scan-ingestion"
...
inputs = {
  ...
  iam_instance_profile = dependency.iam-role-ec2-dpxn-service-layer-scan-ingestion.outputs.iam_instance_profile_arn
  instance_key_name = dependency.key-pair-app.outputs.key_pair_key_name
  min_size = 1
  max_size = 1
  default_cooldown = 300
  desired_capacity = 1
  app_name = "service-layer-scan-ingestion"
  run_list = "role[dpxn_service_layer_scan_ingestion]"
  ...
  asg_tags = flatten([ dependency.common-tags.outputs.asg_common_tags, [
  {
    ...
    "key" = "ServerRole"
    "value" = "service_layer_scan_ingestion"
    "propagate_at_launch" = true
  }]])
}
