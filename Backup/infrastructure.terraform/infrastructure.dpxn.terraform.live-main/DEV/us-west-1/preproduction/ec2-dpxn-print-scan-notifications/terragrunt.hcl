locals {
  ...
  chef_environment = lower(local.environment_vars.locals.chef_environment)
...
dependency "iam-role-ec2-dpxn-print-scan-notifications" {
  config_path = "${get_terragrunt_dir()}/../iam-role-ec2-dpxn-print-scan-notifications"

  mock_outputs_allowed_terraform_commands = ["validate", "plan", "destroy"]
  mock_outputs = {
    iam_role_arn = "arn::"
    iam_instance_profile_id = "12344"
...
inputs = {
  ...
  iam_instance_profile = dependency.iam-role-ec2-dpxn-print-scan-notifications.outputs.iam_instance_profile_arn
  instance_key_name    = dependency.key-pair-app.outputs.key_pair_key_name
  min_size             = 1
  max_size             = 1
  default_cooldown     = 300
  desired_capacity     = 1
  app_name             = "print-scan-notifications"
  run_list             = "role[dpxn_print_scan_notifications]"
  ...
  security_groups      = [dependency.vpc-info.outputs.baseline_sg_id, dependency.vpc-info.outputs.app_sg_id]
