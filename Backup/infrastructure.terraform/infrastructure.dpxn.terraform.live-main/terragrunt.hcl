...
provider "aws" {
  ...
  default_tags {
    tags = {
      GitHash = "${run_cmd("${get_parent_terragrunt_dir()}/scripts/git_data.sh", "hash")}"
      GitBranch = "${run_cmd("${get_parent_terragrunt_dir()}/scripts/git_data.sh", "branch")}"
    }
  }
  assume_role {
