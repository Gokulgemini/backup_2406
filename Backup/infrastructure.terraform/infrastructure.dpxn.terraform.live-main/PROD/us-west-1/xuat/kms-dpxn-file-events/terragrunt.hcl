...
inputs = {
  ...
  kms_alias           = "${local.infra_environment}-${local.app_environment}-file-events"
  additional_policy = [{
    sid = "AllowCrossAccountUseResque"
    actions = [
      "kms:Decrypt",
      "kms:GenerateDataKey"
    ]
    ...
    principals = [{
      type = "AWS"
      identifiers = [
        "arn:aws:iam::${local.pci_account_number}:role/resque-${replace(local.infra_environment, "-", "_")}-${local.app_environment}-role"
      ]
    }]
    resources = [
      "*"
