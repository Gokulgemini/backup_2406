locals {
  ...
  topic_name = "${local.infra_environment}-${local.app_environment}-cml-route"
  # Length of IAM role created by module is not in the range of 1-64 if we use actual name of sns topic name.
  # so, we are using cml-router instead of commercial-route-topic
...
inputs = {
  ...
  kms_master_key_id = dependency.kms-dpxn-commercial-router.outputs.kms_id
