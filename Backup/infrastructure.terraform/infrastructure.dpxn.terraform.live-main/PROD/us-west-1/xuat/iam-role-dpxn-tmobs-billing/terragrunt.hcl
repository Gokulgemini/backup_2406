...
dependency "sqs-dpxn-tmobs-billing" {
  config_path = "${get_terragrunt_dir()}/../sqs-dpxn-tmobs-billing"
...
dependency "logstream-dpxn-tmobs-billing-cloudwatch-log-group" {
...
inputs = {
  ...
  create_role = true
  create_policy = true
  role_name = "${local.infra_environment}-${local.app_environment}-tmobs-billing-role"
  role_description = "Role used by DLX tmobs Billing Service (${local.app_environment})"
  policy_name = "${local.infra_environment}-${local.app_environment}-tmobs-billing-policy"
  policy_description = "Policy used by DLX tmobs Billing Role (${local.app_environment})"
  role_path = "/"
  policy_statement = templatefile("policy/statement.tftpl", {
           sqs-dpxn-tmobs-billing-queue-arn = dependency.sqs-dpxn-tmobs-billing.outputs.queue_arn,
           s3-dpxn-tmobs-billing-bucket-arn = dependency.s3-dpxn-tmobs-billing.outputs.s3_bucket_arn,
					 ...
					 logstream-dpxn-tmobs-billing-cloudwatch-log-group-arn = dependency.logstream-dpxn-tmobs-billing-cloudwatch-log-group.outputs.cloudwatch_log_group_arn})
  oidc_fully_qualified_subjects = [
    "system:serviceaccount:reporting:tmobs-billing"
  ]
