...
dependency "iam-role-ec2-dpxn-check-order" {
  config_path = "${get_terragrunt_dir()}/../iam-role-ec2-dpxn-check-order"
...
inputs = {
  ...
  iam_instance_profile = dependency.iam-role-ec2-dpxn-check-order.outputs.iam_instance_profile_arn
  instance_key_name = dependency.key-pair-app.outputs.key_pair_key_name
  min_size = 1
  max_size = 1
  default_cooldown = 300
  desired_capacity = 1
  app_name = "check-order"
  run_list = "role[dpxn_check_order]"
