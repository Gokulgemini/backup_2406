locals {
  ...
  bucket_name = "dpxn-${local.infra_env}-${local.app_environment}-interim"
...
inputs = {
  ...
  additional_statements = [{
      sid = "AllowCrossAccountResqueRole"
      actions = [
        "s3:GetObject",
        "s3:GetObjectAcl",
      ]
      ...
      principals = [{
        type = "AWS"
        identifiers = [
          "arn:aws:iam::${local.account_vars.locals.pci_account_number}:role/resque-${replace(local.infra_env,"-","_")}-${local.app_environment}-role"
        ]
      }]
      resources = [
    ...
    {
      sid = "AllowCrossAccountPreflightRole"
      actions = [
        "s3:PutObject",
        "s3:PutObjectAcl",
        "s3:PutObjectTagging"
