...
terraform {
  source = "git::git@github.com:verifyvalid/infrastructure.dpx_fulfillment.terraform.modules.git//iam-role-with-policy?ref=${local.modules_version}"
...
dependency "kms-sqs-check-print" {
  config_path = "${get_terragrunt_dir()}/../kms-sqs-check-print"
...
inputs = {
  policy_name = "check-print-${local.env}-${local.app_environment}-policy"
  policy_description = "Used for the Check Print service (${local.app_environment})"
  policy_statement = <<EOF
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Resource": ["${dependency.kms-db-key.outputs.kms_arn}", "${dependency.kms-non-db-key.outputs.kms_arn}", "${dependency.kms-sqs-check-print.outputs.kms_arn}"]
    },
    {
...
EOF
  role_name = "check-print-${local.env}-${local.app_environment}-role"
...
EOF
  create_instance_profile = 1
  app_name = "check_print"
