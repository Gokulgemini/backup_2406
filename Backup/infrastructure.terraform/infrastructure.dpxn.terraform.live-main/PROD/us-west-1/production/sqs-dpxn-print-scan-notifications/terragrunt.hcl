locals {
  ...
  queue_name       = "${local.chef_environment}-${local.app_environment}-print-scan-notifications"
  ...
  tags                        = merge(dependency.common-tags.outputs.common_tags, { "TerraformPath" = path_relative_to_include() })
   policy                      = [{
    ...
    condition = [{
      ...
      values = ["arn:aws:s3:::${local.infra_env}-${local.app_environment}-print-scan-notifications"]
    }]
  }]
}
