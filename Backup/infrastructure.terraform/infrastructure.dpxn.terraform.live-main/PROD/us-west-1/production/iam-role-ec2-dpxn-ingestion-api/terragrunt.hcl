...
dependency "s3-dpxn-api" {
  config_path = "${get_terragrunt_dir()}/../s3-dpxn-api"
...
inputs = {
  policy_name = "pen-api-ingestion-${local.env}-${local.app_environment}-policy"
  policy_description = "Used for the PEN API Ingestion service (${local.app_environment})"
  ...
  "Statement": [
    {
      ...
      "Effect": "Allow",
      "Resource": [
        "${dependency.s3-dpxn-api.outputs.s3_bucket_arn}/*"
      ]
    },
    {
      "Effect": "Allow",
      ...
      "Resource": [
        ...
        "${dependency.kms-dpxn-api.outputs.kms_arn}"
      ]
    },
    {
...
EOF
  role_name = "pen-api-ingestion-${local.env}-${local.app_environment}-role"
