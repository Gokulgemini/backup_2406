...
dependency "sqs-dpxn-scan-ingestion" {
  ...
  mock_outputs = {
    queue_arn = "arn::"
    queue_id = "12344"
    queue = "s3-dpxn-scan-ingestion"
    deadletter_queue_arn = "arn::"
    deadletter_queue_id = "1243"
    deadletter_queue = "s3-dpxn-scan-ingestion-dead"
...
inputs = {
  function_name = "${local.env}-${local.app_env}-dpxn-scan-ingestion"
  bucket_name = dependency.s3-dpxn-scan-ingestion.outputs.s3_bucket_id
  queue_url = dependency.sqs-dpxn-scan-ingestion.outputs.queue_url
  function_filter_suffix = ".IMG"
  memory_size = 256
