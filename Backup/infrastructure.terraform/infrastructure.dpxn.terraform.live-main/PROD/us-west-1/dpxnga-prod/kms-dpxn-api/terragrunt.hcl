...
inputs = {
  ...
  kms_alias = "${local.infra_env}-${local.app_environment}-dpxn-api"
  ...
  additional_policy = [{
    ...
    principals = [{
      ...
      type = "AWS"
      identifiers = ["arn:aws:iam::${local.pci_account_number}:role/${local.infra_env}-${local.app_environment}-preflight-role"]
    }]
    resources = [
      "*"
    ]
