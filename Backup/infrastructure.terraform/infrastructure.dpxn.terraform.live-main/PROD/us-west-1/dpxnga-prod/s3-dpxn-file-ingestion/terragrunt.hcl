locals {
  ...
  bucket_name = "${lower(local.infra_env)}-${local.app_environment}-file-ingestion"
...
inputs = {
  ...
  sqs_notifications = {
    sqs-dpxn-file-ingestion = {
      events = ["s3:ObjectCreated:*"]
      queue_arn = dependency.sqs-dpxn-file-ingestion.outputs.queue_arn
    }
  }
  kms_master_key_id = dependency.kms-dpxn-file-ingestion.outputs.kms_arn
