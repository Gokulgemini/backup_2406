...
dependency "sqs-dpxn-acknowledgement" {
  ...
  mock_outputs = {
    queue_arn = "arn::"
    queue_id = "12344"
    queue = "s3-dpxn-acknowledgement"
    deadletter_queue_arn = "arn::"
    deadletter_queue_id = "1243"
    deadletter_queue = "s3-dpxn-acknowledgement-dead"
...
inputs = {
  function_name = "${local.env}-${local.app_env}-dpxn-acknowledgement"
  bucket_name = dependency.s3-dpxn-acknowledgement.outputs.s3_bucket_id
  queue_url = dependency.sqs-dpxn-acknowledgement.outputs.queue_url
  function_filter_suffix = ".csv"
  memory_size = 256
  policy_statement = <<EOF
{
  ...
  "Statement": [
      ...
      "Effect": "Allow",
      "Resource": "${dependency.kms-dpxn-acknowledgement.outputs.kms_arn}"
    }
  ]
}
EOF
