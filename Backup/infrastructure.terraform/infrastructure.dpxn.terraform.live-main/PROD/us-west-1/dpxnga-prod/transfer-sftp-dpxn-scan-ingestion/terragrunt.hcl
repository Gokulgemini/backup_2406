locals {
  ...
  env               = replace(local.environment_vars.locals.chef_environment, "_", "-")
...
terraform {
  source = "git::git@bitbucket.org:deluxe-development/aws-sftp.git//?ref=${local.modules_version}"
}
...
inputs = {
  ...
  endpoint_type           = "VPC"
  iam_role_name           = "${local.env}-${local.app_env}-scan"
  kms_key_arn             = dependency.kms-dpxn-scan-ingestion.outputs.kms_arn
