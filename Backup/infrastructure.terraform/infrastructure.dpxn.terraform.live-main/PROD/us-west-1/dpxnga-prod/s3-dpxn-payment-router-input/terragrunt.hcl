locals {
  ...
  bucket_name       = "${local.infra_environment}-${local.app_environment}-payment-router-input"
...
inputs = {
  ...
  kms_master_key_id  = dependency.kms-dpxn-payment-router.outputs.kms_arn
  encryption_policy  = true
  sqs_notifications = {
    sqs-dpxn-payment-router-input = {
      events    = ["s3:ObjectCreated:*"]
      queue_arn = dependency.sqs-dpxn-payment-router-input.outputs.queue_arn
    }
  }
