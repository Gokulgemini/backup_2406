...
inputs = {
  ...
  cluster_name    = "${local.infra_environment}-${local.app_environment}-payment-router-redis"
  ...
  cidr_blocks     = concat(dependency.subnet-details-db.outputs.subnet.*.cidr_block, dependency.subnet-details-app.outputs.subnet.*.cidr_block)
