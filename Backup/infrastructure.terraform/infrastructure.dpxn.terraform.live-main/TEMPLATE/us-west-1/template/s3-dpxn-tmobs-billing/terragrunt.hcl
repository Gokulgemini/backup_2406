locals {
  ...
  bucket_name = "${local.infra_environment}-${local.app_environment}-tmobs-billing"
...
inputs = {
  ...
  kms_master_key_id = dependency.kms-dpxn-tmobs-billing.outputs.kms_arn
  encryption_policy = true
  sqs_notifications = {
    sqs-dpxn-tmobs-billing = {
      events = ["s3:ObjectCreated:*"]
      queue_arn = dependency.sqs-dpxn-tmobs-billing.outputs.queue_arn
    }
  }
