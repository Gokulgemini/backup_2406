locals {
  ...
  bucket_name = "${local.infra_environment}-${local.app_environment}-commercial-router"
...
inputs = {
  ...
  kms_master_key_id = dependency.kms-dpxn-commercial-router.outputs.kms_arn
  encryption_policy = true
  sqs_notifications = {
    sqs-dpxn-commercial-router = {
      events = ["s3:ObjectCreated:*"]
      queue_arn = dependency.sqs-dpxn-commercial-router.outputs.queue_arn
    }
  }
