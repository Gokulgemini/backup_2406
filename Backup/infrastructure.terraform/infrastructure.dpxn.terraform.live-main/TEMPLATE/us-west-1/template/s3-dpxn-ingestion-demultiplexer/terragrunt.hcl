locals {
  ...
  infra_environment = replace(local.environment_vars.locals.infra_environment,"_", "-")
  app_environment = local.environment_vars.locals.app_environment
  bucket_name = "${lower(local.infra_environment)}-${local.app_environment}-ingestion-demultiplexer"
...
dependency "common-tags" {
  ...
  mock_outputs = {
    ...
    common_tags = {
      "AppName" = "ingestion-demultiplexer"
    }
  }
}
...
inputs = {
  ...
  sqs_notifications = {
    sqs-dpxn-ingestion-demultiplexer = {
      events = ["s3:ObjectCreated:*"]
      queue_arn = dependency.sqs-dpxn-ingestion-demultiplexer.outputs.queue_arn
    }
  }
  kms_master_key_id = dependency.kms-dpxn-ingestion-demultiplexer.outputs.kms_arn
