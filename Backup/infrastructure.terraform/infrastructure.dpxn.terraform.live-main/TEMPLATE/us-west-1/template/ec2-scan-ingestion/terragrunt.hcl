...
dependency "iam-role-ec2-dpxn-scan-ingestion" {
  config_path = "${get_terragrunt_dir()}/../iam-role-ec2-dpxn-scan-ingestion"
...
inputs = {
  ...
  iam_instance_profile = dependency.iam-role-ec2-dpxn-scan-ingestion.outputs.iam_instance_profile_arn
  instance_key_name = dependency.key-pair-app.outputs.key_pair_key_name
  min_size = 4
  max_size = 4
  default_cooldown = 300
  desired_capacity = 4
  app_name = "scan-ingestion"
  run_list = "role[dpxn_scan_ingestion]"
