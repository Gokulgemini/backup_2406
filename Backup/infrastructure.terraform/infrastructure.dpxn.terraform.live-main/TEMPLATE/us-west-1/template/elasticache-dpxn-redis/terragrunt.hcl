...
terraform {
  source = "git::git@bitbucket.org:deluxe-development/aws-elasticache-redis-non-clustered.git//?ref=${local.modules_version}"
}
...
inputs = {
  ...
  auth_token_arn  = dependency.secrets.outputs.arns["${local.infra_environment}/${local.app_environment}/${local.app_name}/backend/redis/authToken"]
