locals {
  ...
  env               = lower(replace(local.environment_vars.locals.chef_environment, "_", "-"))
...
inputs = {
  ...
  create_role             = true
  create_policy           = true
  role_name               = "${local.env}-${local.app_environment}-service-layer-scan-ingestion"
  role_description        = "Role used by DPXN Service Layer Scan Ingestion Service (${local.app_environment})"
  policy_name             = "${local.env}-${local.app_environment}-service-layer-scan-ingestion"
  policy_description      = "Used for the Service Layer Scan Ingestion service (${local.app_environment})"
  ...
  oidc_fully_qualified_subjects   = [
    "system:serviceaccount:print-scan-service-layer:dlx-service-layer-scan-ingestion"
  ]
