locals {
  ...
  bucket_name      = "${local.chef_environment}-${local.app_environment}-print-scan-notifications"
...
inputs = {
  ...
  kms_master_key_id  = dependency.kms-dpxn-print-scan-notifications.outputs.kms_arn
  tags = merge(dependency.common-tags.outputs.common_tags, {
    "TerraformPath" = path_relative_to_include()
    "Name"          = local.bucket_name
  })
  sqs_notifications  = {
    sqs-dpxn-print-scan-notifications = {
      events    = ["s3:ObjectCreated:*"]
      queue_arn = dependency.sqs-dpxn-print-scan-notifications.outputs.queue_arn
    }
  }
}
