...
namespace RDM.Messaging.ImageVault.Tests
{
    ...
    public class VerifyImageSizeMessageUnitTests
    {
        private readonly RequestIdentifier _requestId;
        private readonly ImageId _imageId;

        public VerifyImageSizeMessageUnitTests()
        {
        ...
        [Fact]
        [Trait("Category", "Unit")]
        public void VerifyImageSizeMessage_Serialize_ToString()
        {
            // Arrange
        ...
        [Fact]
        [Trait("Category", "Unit")]
        public void VerifyImageSizeMessage_Deserialize_FromString()
        {
            // Arrange
            var item = CreateTestMessage();
            ...
            // Act
            var result = JsonConvert.DeserializeObject<VerifyImageSizeMessage>(json);
        ...
        {
            // Act
            var exception = Record.Exception(() => new VerifyImageSizeMessage(_requestId, null));
        ...
        {
            // Act
            var exception = Record.Exception(() => new VerifyImageSizeMessage(_requestId, ImageId.Empty));
        ...
        {
            // Act
            var exception = Record.Exception(() => new VerifyImageSizeMessage(null, _imageId));
        ...
        {
            // Act
            var exception = Record.Exception(() => new VerifyImageSizeMessage(RequestIdentifier.Empty, _imageId));
        ...
        {
            // Act
            var item = Activator.CreateInstance<VerifyImageSizeMessage>();

            // Assert
            Assert.Equal(VerifyImageSizeMessage.RabbitExchange, item.ExchangeName);
            Assert.Equal(VerifyImageSizeMessage.RabbitQueue, item.DefaultQueueName);
            Assert.Equal(VerifyImageSizeMessage.RabbitKey, item.RoutingKey);
        }
        ...
        public void Ctor_ValidParameters_ObjectCreated()
        {
            ...
            // Act
            var item = new VerifyImageSizeMessage(_requestId, _imageId);
        ...
        private VerifyImageSizeMessage CreateTestMessage(bool variant = false)
        {
            return new VerifyImageSizeMessage(_requestId, variant ? new ImageId("77fbc7618e5c47a2a181ae5a9a74a01e") : new ImageId("97c921f0ecfc411b9997bd6720ceb26b"));
        }
    }
}
